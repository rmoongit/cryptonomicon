...
export default {
  ...
  computed: {
    filteredTips() {
      const filter = this.coinList
        .filter((coin) =>
          coin.Symbol.toLowerCase().includes(this.ticker.toLowerCase().trim()),
        )
        .slice(0, 4)
  ...
  methods: {
    //Калькулируем ширину графов при ресайзе
    ...
    checkIsAddedTicker(currentTip) {
      return this.tickers.some(
        (tip) => tip.name.toLowerCase() === currentTip.name.toLowerCase(),
      )
    },
